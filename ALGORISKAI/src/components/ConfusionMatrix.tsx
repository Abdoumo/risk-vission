...
export default function ConfusionMatrix({ data }: ConfusionMatrixProps) {
  ...
  return (
    <div className="rounded-2xl border border-white/5 bg-slate-900/40 backdrop-blur-md p-6 transition-all duration-300 hover:border-emerald-500/20 hover:shadow-[0_0_30px_rgba(16,185,129,0.05)] h-full">
          ...
          {data.valeurs.map((row, i) => (
            <div key={i} className={`flex items-center gap-1.5 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              {row.map((val, j) => (
                <div
                  key={j}
                  className={`flex h-28 w-28 flex-col items-center justify-center rounded-xl border border-white/5 transition-all duration-300 hover:scale-105 ${i === j ? 'hover:shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:border-emerald-500/30' : 'hover:shadow-[0_0_20px_rgba(244,63,94,0.2)] hover:border-rose-500/30'}`}
                  style={{ backgroundColor: getColor(val, i, j) }}
                >
                  <span className={`text-3xl font-black ${i === j ? 'text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.5)]'}`}>{val}</span>
                  <span className={`mt-1.5 text-[11px] font-bold ${i === j ? 'text-emerald-200/70' : 'text-rose-200/70'}`}>{((val / total) * 100).toFixed(1)}%</span>
                  <span className={`text-[10px] font-medium mt-0.5 ${i === j ? 'text-emerald-500/80' : 'text-rose-500/80'}`}>{data.labels[i * 2 + j]}</span>
                </div>
              ))}
            </div>
          ))}
